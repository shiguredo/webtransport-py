...
さらに、サーバー側の高レベル拒否 API (`on_session_request` コールバック、0134) が Origin 検証・URI パス検証を行うには、SESSION_READY イベントが受信した HTTP ヘッダーを高レベル層へ渡す必要がある。現状 `pending_headers_` は SESSION_READY push 直後に消去され、ヘッダーはどこにも載らない。本 issue は 0134 の先行必須として、SESSION_READY イベントに受信 HTTP ヘッダーを載せる拡張も行う。
...
- `H2Event` 構造体 (`src/bindings/webtransport_h2.h`) には `type` / `session_id` / `stream_id` / `data` / `error_code` / `error_message` / `fin` の 7 フィールドがあり、`status_code` も `headers` も無い
- SESSION_READY イベントの push は 2 箇所にある (サーバー側: CONNECT リクエストの受理、クライアント側: 200 応答の受信)。どちらも push 直後の `pending_headers_.erase(it)` でヘッダーを消しており、イベントに載せていない。なおクライアント側の成功判定は現状 `:status == "200"` のみであり、2xx 非 200 (201 等) に対する成功判定の拡張は 0104 のスコープ
- `src/webtransport/h2.pyi` の `EventType` / `Event` にも `SESSION_REJECTED` / `status_code` / `headers` は無い (h2.pyi は nanobind のスタブ生成と `make develop` のコピーによる生成物で git 管理外であり、実体は bindings 側の nanobind 定義。型スタブの出所を bindings 側で制御するため、手編集しない)
...
- 対称性の参考: `H3EventType` / `H3Event` (`src/bindings/webtransport_h3.h`) にも同種の `SessionRejected` / `status_code` は存在しない。HTTP/3 版 (0112 で扱う `h3.Client.connect()` の 2xx 未待機バグ) が別方式を採らず HTTP/2 と同型で対応するかは 0112 側で判断する (本 issue は HTTP/2 のみを扱う)

## 設計方針

- `src/bindings/webtransport_h2.h` の `H2EventType` 列挙に `SessionRejected` を追加する。**末尾 (Error の後) に追加し、既存 8 バリアントに割り当てられる数値 (0-7) を変えない** (h2.pyi の `EventType` は数値リテラルを公開しており、値を変えると乖離する。h3 側 (`H3EventType`) も `Error` が末尾であり、中間挿入による数値シフトで pyi の数値リテラルと乖離する事故を起こさない)
- `src/bindings/webtransport_h2.h` の `H2Event` 構造体に `uint16_t status_code = 0;` と `std::vector<std::pair<std::string, std::string>> headers;` フィールドを追加する。`status_code` は `SessionRejected` 発火時のみ意味を持ち、他のイベント種別ではデフォルト値 `0` のまま利用される。`headers` は `SessionReady` 発火時のみ意味を持ち、他のイベント種別では空のまま利用される (この意図を `.h` の該当フィールド上にコメントで明記する)
- `src/bindings/webtransport_h2.cpp` の `on_frame_recv_callback` の非 2xx 応答分岐 (`wt_sessions_.erase(stream_id)` の直前) で `H2EventType::SessionRejected` イベントを push し、`event.session_id` にはセッション ID、`event.status_code` には受信した HTTP status code (例: 403) を載せる。`event.error_code` はデフォルト `0` のまま (nghttp2 の HTTP/2 error code を載せる既存 SessionClosed 経路とは意味論が異なるため、status_code フィールドに載せる)
- **status_code のパース方針**: `std::stoi` は例外を投げる可能性があり、`on_frame_recv_callback` は nghttp2 の C ABI コールバックのため C++ 例外を伝播させると未定義動作になる。したがって `<charconv>` の `std::from_chars` (noexcept) を使う。nghttp2 は `:status` を 100-999 の 3 桁数字 (1 桁目が '1'-'9') としてバリデーション済みだが、HTTP status code として意味を持つ 100-599 の範囲外 (600-999 等) は `0` に丸める (不正な status code をアプリが観測することを避ける)
- SESSION_READY イベントを push する 2 箇所 (サーバー側 CONNECT リクエストの受理 / クライアント側 2xx 応答の受信) で、push 時点の `pending_headers_` (受信順の name / value のリスト) を `event.headers` へコピーする。`pending_headers_.erase(it)` の前にコピーすること
  - サーバー側のヘッダーは CONNECT リクエストのヘッダー (`:method` / `:protocol` / `:scheme` / `:authority` / `:path` / `origin` 等)、クライアント側のヘッダーは 2xx 応答のヘッダー (`:status` / `webtransport-init` 等) である (0134 の `on_session_request` が参照するのはサーバー側)
...
- `src/webtransport/h2.pyi` の変更は bindings 側の nanobind 定義 (`bind_webtransport_h2`) だけで行う (`.value` の追加と `.def_ro` の docstring 引数)。h2.pyi を手編集しない (次回ビルドで即生成物に上書きされ、コミットにも残らない)

## 完了条件

- `src/bindings/webtransport_h2.h` の `H2EventType` に `SessionRejected` バリアントが末尾 (Error の後) に追加されている
- `src/bindings/webtransport_h2.h` の `H2Event` に `uint16_t status_code = 0;` と `std::vector<std::pair<std::string, std::string>> headers;` フィールドが追加され、「SessionRejected 発火時のみ status_code が意味を持つ / SessionReady 発火時のみ headers が意味を持つ」旨のコメントが添えられている
- `src/bindings/webtransport_h2.cpp` の非 2xx 応答分岐で `SessionRejected` イベントが push され、`status_code` にパース済みの HTTP status code (403 等) が載る。パース失敗・範囲外は `0` が載る (実行時に `std::from_chars` を使うこと)
- SESSION_READY イベント (サーバー側・クライアント側の両方) の push 時に、受信した HTTP ヘッダーが `event.headers` に載る
...
- `src/webtransport/h2.pyi` (生成物) に `EventType.SESSION_REJECTED`、`Event.status_code: int`、`Event.headers: list[tuple[str, str]]` プロパティが現れ、docstring に「SessionRejected 発火時のみ HTTP status code。他イベントでは 0」「SessionReady 発火時のみ受信 HTTP ヘッダー。他イベントでは空」旨が記載され、`ty check` を通る
...
- SESSION_READY イベントの `headers` に受信 HTTP ヘッダー (CONNECT リクエストのヘッダー等) が載ることも同じ Sans-IO パターンで検証する
- 1xx を挟んだ拒否ではセッション ID が残る既知の制約と同様に SESSION_REJECTED も発火しない (設計方針通り。この制約を固定する新規テストは追加しない)
- `AGENTS.md` のモック禁止に従い、実 `h2_low.Session` 対で Sans-IO テストを追加する (既存 `test_client_non_2xx_reject_removes_session` と同じパターン)。追加するテストには意図・前提・期待値を日本語コメントで明記する
...
- **後続 (0134 の先行必須)**: 0134 (`h2.Server に拒否 API を追加する`) は本 issue のマージを前提とする。0134 の `on_session_request` コールバックの `headers` 引数は本 issue が追加する `Event.headers` (SessionReady 時の受信 HTTP ヘッダー) を利用する。実装順序は 0133 (本 issue) → 0134 の順に固定する
...
- **関連 (対象領域重複)**: 0104 (`h2.Client が 2xx 非 200 応答をセッション確立として扱わない問題`) は本 issue と同じ `on_frame_recv_callback` の HCAT_RESPONSE 分岐 (`is_success` 判定) を変更する。双方の実装者が同じ分岐を触るため、実装順序とマージ後の整合について認識を揃える
