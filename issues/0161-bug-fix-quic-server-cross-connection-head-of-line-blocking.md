...
- 受信ループは受信と投入のみ行い、アプリコールバックを接続ごとの単独タスクで順序どおり実行する。終了と停止時に寿命を管理し、例外時は記録後に当該接続を閉じる
- 送信は毎反復で全接続を走査し、受信の有無にかかわらず送出とタイマーを処理する。接続ごとに例外を隔離する
- `tests/test_e2e_quic_isolation.py` に 6 件のテスト (2 台分離・100 台分離・終了到達・停止後空・再入停止・例外隔離) を追加する
- 全 956 件のテストが通過することと、レビュー 4 周で致命的と重要が 0 件であることを確認した
