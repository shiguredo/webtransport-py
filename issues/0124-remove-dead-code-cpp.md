...
- **空実装コールバック**: `QuicConnection::acked_stream_data_offset_cb` (`src/bindings/quic.cpp`) は return 0 のみで、コメントと実装が食い違う。宣言は `src/bindings/quic.h`、登録は `src/bindings/quic.cpp` の 3 箇所にある
...
- `acked_stream_data_offset_cb` は宣言 (`src/bindings/quic.h`)・定義・登録 3 箇所 (`src/bindings/quic.cpp`) をすべて削除する
