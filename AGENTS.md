...
- 問題が起きて一定数時間がかかり始めたら /issues に登録すること
- Issue は md ファイルとして管理すること
- Issue は日本語で書くこと
- Issue は具体的に書くこと
- Issue は再現手順を書くこと
- Issue は期待する動作を書くこと
...
- pytest を利用したテストコードによる再現を最優先すること

## RFC

- 仕様がわからなくなったら docs/ 以下に RFC ドキュメントがあるのでそれを確認し準拠すること

## 依存ライブラリについて

- `_deps/` 以下に依存ライブラリのソースコードがあるので最優先で確認すること
...
- Free-Threading 前提なので GIL は存在しない
...
テストはベンチマークをのぞけば遅くとも 30 秒以内に終わるので 30 秒経過したらタイムアウトにする。

- メモリーリークは必ず解決すること
  - NANOBIND_SUPPRESS_LEAK_WARNINGS を利用しないこと
- モックやスタブは利用禁止
...
- ベンチマークは `tests/` 以下に配置すること
- ベンチマークファイルは `test_bench_` prefix を持つファイルのみが実行される
- ベンチマークを実行するには `--benchmark-only` オプションで有効化する
...
- nghttp3_conn は mutex の保護が必要
- 別の nghttp3_conn はそれぞれの thread でもよい
